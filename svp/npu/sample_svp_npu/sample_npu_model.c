...
td_void sample_npu_output_model_result(td_u32 model_index,stYolovDetectObjs* pOut) {
    ...
    for (i = 0; i < aclmdlGetDatasetNumBuffers(g_npu_acl_model[model_index].output_dataset); ++i) {
        ...
        ncnn_result_yolov8(data, len/sizeof(float), pOut);
    }
...
td_void sample_npu_FEATURE_output_model_result(td_u32 model_index, stObjinfo *p_objs) {
    aclDataBuffer *data_buffer = TD_NULL;
    td_void *data = TD_NULL;
    td_u32 len;
    td_u32 i, j;

    int buffer_number = aclmdlGetDatasetNumBuffers(g_npu_acl_model[model_index].output_dataset);
    ...
    for (i = 0; i < aclmdlGetDatasetNumBuffers(g_npu_acl_model[model_index].output_dataset); ++i) {
        ...
        // printf("buffer_number[%d], output len:%d\n ",buffer_number, len);  // exit(1);
        if (data == TD_NULL || len == 0) {
            sample_svp_trace_err("get data null.\n");
            continue;
        }
        memcpy(p_objs->feature, data, len);
    }
